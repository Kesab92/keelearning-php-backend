...
class QuestionsIndexCardsImporter extends Importer
{
    ...
    protected function importData($additionalData, $headers, $questions)
    {
        ...
        DB::transaction(function () use ($questions, $headers, $category, $app, $questionsCount, &$idx, $accessLogEngine, $creatorId) {
            foreach ($questions as $questionData) {
                $question = new Question();
                $question->category_id = $category->id;
                $question->visible = true;
                $question->app_id = $category->app_id;
                $question->type = Question::TYPE_INDEX_CARD;
                $question->save();
